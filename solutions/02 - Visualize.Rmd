...
Now, what if we wanted to change the general "look" of of the non-data elements, like the background color and whether the grid lines are included, using a pre-defined theme? There are several of these to choose from, listed here: https://ggplot2.tidyverse.org/reference/ggtheme.html.  First, install the package "ggthemes" using install.package, and load it.  Then plot and add a theme.  We've done this work for you below, so you can experiment with it.
